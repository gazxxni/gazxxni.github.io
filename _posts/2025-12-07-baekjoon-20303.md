...
### 풀이 핵심 로직
이 문제는 그래프를 구성하여 각 아이들을 정점으로, 친구 관계를 간선으로 나타내는 문제입니다. 스브러스가 뺏을 수 있는 사탕의 최댓값을 구하기 위해, DFS를 통해 각 연결된 컴포넌트(친구 관계로 연결된 아이들의 집합)의 크기와 사탕의 총합을 계산한 후, 이를 배낭 문제 형태로 변환하여 최대 사탕의 양을 구합니다.

### 동작 과정
1. **입력 처리**: N(아이 수), M(친구 관계 수), K(울음소리 기준 아이 수)를 입력받고, 각 아이가 받은 사탕의 수와 친구 관계를 읽어들입니다.
   - 예시 입력:
     ```
     5 4 2
     1 2 3 4 5
...
2. **그래프 구성**: 각 아이의 친구 관계를 그래프 형태로 구성합니다. 리스트를 사용하여 각 아이의 친구를 저장합니다.

3. **DFS를 통한 컴포넌트 탐색**: `dfs` 함수를 통해 방문하지 않은 아이를 찾아 그와 친구인 아이들을 모두 탐색하면서 연결된 컴포넌트의 크기와 사탕의 총합을 구합니다. 이 과정에서 방문한 아이들은 `visited` 배열을 통해 관리합니다.
   - 예를 들어, 아이 1을 방문하면 아이 2, 3, 4, 5까지 모두 탐색하면서 사탕 수와 크기를 누적합니다.

4. **배낭 문제 해결**: 각 컴포넌트의 크기와 사탕의 총합을 바탕으로, K-1 이하의 크기로 뺏을 수 있는 사탕의 최대값을 구하기 위해 동적 프로그래밍(DP)을 사용합니다.
   - DP 배열을 초기화하고, 각 컴포넌트에 대해 크기와 값을 고려하여 DP 테이블을 갱신합니다.

5. **결과 출력**: DP 테이블에서 최댓값을 찾아 출력합니다.

### 시간 복잡도
- DFS 탐색: O(N + M) (모든 아이와 친구 관계를 한 번씩 탐색)
- DP 처리: O(Capacity * C) (Capacity는 K-1, C는 컴포넌트의 수, 최악의 경우 N에 비례)
- 전체 시간 복잡도: O(N + M + K * C) (실제로는 N과 M이 주로 영향을 미침)
