...
### 풀이 핵심 로직
이 문제의 핵심 아이디어는 BFS(너비 우선 탐색) 알고리즘을 사용하여 상근이가 빌딩 내부를 탐색하고, 문을 열 수 있는 열쇠를 얻으며 문서의 수를 세는 것입니다. 상근이는 처음에 가지고 있는 열쇠를 바탕으로 탐색을 시작하고, 문을 열 수 있는 열쇠를 발견하면 해당 문이 있는 위치를 다시 탐색할 수 있도록 대기열에 저장합니다.

### 동작 과정
1. **입력 처리**: 입력으로 주어진 테스트 케이스의 수와 각 테스트 케이스에 대한 빌딩의 맵과 초기 열쇠를 읽어옵니다.
2. **BFS 초기화**: BFS를 위해 시작 위치(빌딩의 외부)에서부터 탐색을 시작합니다. 이때, 방문한 위치는 `visited` 집합에 저장하여 중복 방문을 방지합니다.
3. **탐색 진행**:
   - 현재 위치에서 상하좌우로 이동하며 각 위치의 정보를 확인합니다.
   - 빈 공간(`.`)이나 문서(`$`)일 경우 해당 위치를 방문 처리하고 큐에 추가합니다.
   - 문(`A`, `B`, ...)이 있을 경우, 현재 가지고 있는 열쇠로 그 문을 열 수 있는지 확인합니다. 열 수 없다면, 해당 문을 `waiting_doors`에 저장합니다.
   - 열쇠(`a`, `b`, ...)를 발견하면, 그 열쇠로 열 수 있는 모든 문을 재탐색합니다.
4. **문서 카운트**: 문서를 훔친 경우 카운트를 증가시키고 그 위치를 빈 공간으로 변경하여 중복 카운트를 방지합니다.
5. **결과 출력**: 각 테스트 케이스에 대해 최대 문서 수를 출력합니다.
...
.... 
```
- 상근이는 처음에 외부에서 시작하며, 현재 가지고 있는 열쇠가 없습니다.
- 빌딩 내부에서 문서(`$`)를 발견하고, BFS를 통해 문서의 수를 세어 1을 출력합니다.

### 시간 복잡도
문제의 시간 복잡도는 O(h * w)입니다. 여기서 h는 빌딩의 높이, w는 빌딩의 너비입니다. 각 위치를 최대 한 번씩 방문하기 때문에, 전체 맵을 탐색하는 경우의 수는 h*w가 됩니다. BFS를 통해 문서와 열쇠를 탐색하는 과정에서 발생하는 추가적인 작업도 이 범위 내에 포함되므로, 전체 시간 복잡도는 O(h * w)로 볼 수 있습니다.
