...
### 풀이 핵심 로직
이 문제는 수빈이가 집으로 가는 최단 경로를 찾는 문제로, 일반적인 이동 방법인 점프와 특정 지점 간의 이동인 텔레포트를 활용한다. Dijkstra 알고리즘을 사용하여 시작 위치에서 여러 텔레포트 지점 및 집까지의 최단 거리를 계산하고, 이를 통해 최소 시간을 찾는다.

### 동작 과정
1. **입력 처리**: 수빈이의 시작 위치 `(xs, ys)`와 집의 위치 `(xe, ye)`를 읽고, 3개의 텔레포트 지점을 저장한다.
2. **우선순위 큐 초기화**: 시작 위치에서의 초기 비용을 0으로 설정하고 우선순위 큐에 추가한다.
3. **Dijkstra 알고리즘 실행**:
   - 우선순위 큐에서 현재 위치와 비용을 가져온다.
   - 현재 위치에서 집까지의 직선 거리와 현재까지의 비용을 합쳐 최소 비용을 갱신한다.
   - 현재 위치에서 연결된 텔레포트 지점으로의 이동 비용을 계산하고 업데이트한다.
   - 현재 위치에서 모든 텔레포트 지점까지 점프하는 비용을 계산하고 업데이트한다.
4. **결과 출력**: 최소 비용을 출력한다.
...
5 5 6 6
```
- 수빈이는 (0, 0)에서 (1, 1)로 텔레포트를 사용하여 10초를 소모하고, 그 후 (100, 100)으로 이동할 수 있다.
- 점프를 통해 (100, 100)까지 이동하는 데 걸리는 시간도 계산하여 최종 최소 시간을 결정한다.

### 시간 복잡도
- Dijkstra 알고리즘의 시간 복잡도는 O((V + E) log V)이다. 여기서 V는 노드 수, E는 간선 수를 나타내며, 이 문제에서는 텔레포트 지점과 시작/끝 지점으로 구성된 노드 수에 따라 달라진다. 따라서, 최악의 경우 O(8 log 8)로 대략 상수 시간에 해당한다.
