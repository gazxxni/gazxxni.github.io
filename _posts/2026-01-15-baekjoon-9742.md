...
### 풀이 핵심 로직
주어진 문자열의 순열을 구하기 위해 백트래킹을 사용하여 모든 가능한 순열을 생성하고, 이를 사전순으로 정렬한 후, 요청된 위치에 해당하는 순열을 출력한다. 만약 요청된 위치가 생성된 순열의 수를 초과하면 "No permutation"을 출력한다.

### 동작 과정
1. 입력 예시: `beain 10`
   - 주어진 문자열 `beain`의 길이는 5로, 전체 순열의 수는 5! = 120이다.
   - 입력된 위치 `10`이 120보다 작으므로 순열을 생성한다.

2. 백트래킹을 통해서 다음과 같은 순열을 생성한다:
   - `beain`, `beani`, `beian`, `beina`, `bean`, ... 등등.
   - 모든 순열을 생성한 후 사전순으로 정렬하면 위의 순열들이 정렬된다.

3. 정렬된 순열 리스트에서 10번째 순열을 찾는다. (인덱스는 0부터 시작하므로 `result[9]`를 찾는다)
   - 찾은 순열은 `binae`이고, 이를 출력한다.

4. 만약 위치가 120보다 크다면 "No permutation"을 출력한다.

### 시간 복잡도
이 알고리즘의 시간 복잡도는 O(n!)이다. 이는 n이 10일 때, 최대 10! = 3,628,800개의 순열을 생성해야 하므로, 최악의 경우 모든 순열을 생성하는 데 걸리는 시간이다.
