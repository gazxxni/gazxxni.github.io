...
### 풀이 핵심 로직
이 문제의 핵심 아이디어는 BFS(너비 우선 탐색)를 활용하여 상근이가 가지고 있는 열쇠를 사용해 문을 열고, 문서에 접근하는 경로를 탐색하는 것입니다. 초기에는 상근이가 가진 열쇠에 따라 접근 가능한 공간을 탐색하고, 새로운 열쇠를 발견할 때마다 기다리는 문을 다시 탐색하여 문서의 개수를 최대화합니다.

### 동작 과정
1. **입력 처리**: 각 테스트 케이스에 대해, 빌딩의 크기와 구조를 읽고, 가장자리에 빈 공간(`.`)을 추가하여 BFS 탐색을 쉽게 합니다.
2. **BFS 초기화**: 상근이는 빌딩 외부에서 시작하므로 시작 좌표 `(0, 0)`에서 BFS를 시작합니다. 탐색 중 방문한 좌표를 저장하기 위해 `visited` 집합을 사용합니다.
3. **탐색 과정**:
   - 현재 위치에서 상하좌우로 이동하며 다음 위치를 탐색합니다.
   - 빈 공간(`.`)이나 문서(`$`)는 바로 탐색합니다.
   - 문(`A-Z`)은 열쇠가 있는지 확인 후, 열 수 있는 경우 탐색합니다. 열 수 없는 경우에는 대기 리스트에 저장합니다.
   - 열쇠(`a-z`)를 발견하면 해당 열쇠를 추가하고, 대기 중인 문들을 열 수 있는 경우 다시 탐색합니다.
4. **문서 수 세기**: 문서를 발견할 때마다 카운트를 증가시키고, 중복 카운트를 방지하기 위해 해당 위치를 빈 공간으로 변경합니다.
5. **결과 출력**: 각 테스트 케이스에 대해 문서의 개수를 출력합니다.
...
a
```
- 상근이는 처음 `(0, 0)`에서 BFS를 시작하여 `(1, 0)`으로 이동, 여기에 문서가 있습니다. 열쇠 `a`를 가지고 있으므로 문서 `$`를 훔치고 카운트가 1이 됩니다.

### 시간 복잡도
시간 복잡도는 O(h * w)입니다. 여기서 h는 빌딩의 높이, w는 너비입니다. BFS 탐색을 통해 모든 공간을 한 번씩 방문하기 때문에 이 복잡도로 충분합니다.
